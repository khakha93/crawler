...
# from imvelyProduct.cateInitPage import CateInitPage as ImvCate
from mutnamProduct.cateInitPage import CateInitPage as MutCate
import re


def get_sorted_url(original_url):
    return original_url + '&sort_method=5'
...
class SitePage(Page):
    ...
    def cate_crawling(self, cate_id, cate_url):
        ...
        if self.site_name == 'musinsa':
            ...
            return
        elif self.site_name == 'imvely':
            # cate_init_page = ImvCate(cate_url, self.ch, cate_id)]
            return
        elif self.site_name == 'mutnam':
            # 카테고리
            cate_init_page = MutCate(get_sorted_url(cate_url), cate_id)
        # elif self.site_name == 'uniceleb':
        #     # 사이트 내에서 쓰는 카테고리의 고유번호를 url에서 추춣해서 쓴다. 우리 디비의 category id 는 안 쓸거야.
        #     sub_url = re.compile('cate_no=\\d+').search(cate_url).group()
        #     cate_number = int(re.split('=', sub_url)[1])
        #     cate_init_page = UnicelebCate(get_sorted_url(cate_url), cate_number)
        else:
            print(f'{self.site_name}: 준비가 안 되어있음. sitePage 에서 import 빼먹었어')
            return
