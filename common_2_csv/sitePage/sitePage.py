...
from unicelebProduct.cateInitPage import CateInitPage as UnicelebCate
import re
...
class SitePage(Page):
    ...
    def cate_crawling(self, cate_id, cate_url):
        ...
        if self.site_name == 'musinsa':
            ...
            # 사이트 내에서 쓰는 카테고리의 고유번호를 url에서 추춣해서 쓴다. 디비의 category id 는 안 쓸거야.
            sub_url = re.compile('cate_no=\\d+').search(cate_url).group()
            cate_number = int(re.split('=', sub_url)[1])
            cate_init_page = UnicelebCate(cate_url, self.ch, cate_number)
