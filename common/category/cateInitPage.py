...
class CateInitPage(CategoryPage):
    ...
    def crawling_by_category(self):
        ...
        if self.cr_type == 3:
            ...
            now_page = self.err_page
            self.move_n_th_page(now_page)

        # 페이지 넘겨가면서 크롤링 해준다
        try:
            self.through_page(now_page)
        except TimeoutException:
            # <$$$> 이렇게 말고 비정상 종료를 체크해야하는거 아니냐
            self.fill_err_product('time_out_error', self.url)
            return
        ...
        print(f'Successfully Finish category({self.category}) crawling\n')
        return
